...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    """
    imageid1 = os.path.basename(image_file1)[:-4]
    imageid2 = os.path.basename(image_file2)[:-4]
    matchid = '{}_{}'.format(imageid1, imageid2)

    match_save= os.path.join(RANSAC_MATCH_DIR, matchid + '.npy')
    essential_mtx_save = os.path.join(RANSAC_ESSENTIAL_DIR, matchid + '.npy')
    image_save = os.path.join(RANSAC_MATCH_IMAGE_DIR, matchid + '.png')
    feature_match = os.path.join(BF_MATCH_DIR, matchid + '.npy')

    if not os.path.exists(feature_match):
        return matchid  # images are not matched under feature matching or the image_id1 >= image_id2

    match_idxs = np.load(feature_match)
    if match_idxs.shape[0] < min_feature_matches:
        return matchid  # there are too little inliers  
    points1 = get_selected_points2d(image_id=imageid1, select_idxs=match_idxs[:, 0])
    points2 = get_selected_points2d(image_id=imageid2, select_idxs=match_idxs[:, 1])
    ...
    essential_mtx, mask = cv2.findEssentialMat(
        points1, points2, camera_intrinsics, 
    ...
    np.save(match_save, inlier_idxs)
    np.save(essential_mtx_save, essential_mtx)
    ...
    cv2.imwrite(image_save, save_image)
    return match_id
        ...
        for j in range(i + 1, len(image_ids)):
            ...
            if os.path.exists(match_file):
                ...
                if matches.shape[0] >= min_num_inliers:
                    graph.add_edge(i, j)                # 如果内点数量达标，则认为这两张图有共视关系，添加边
