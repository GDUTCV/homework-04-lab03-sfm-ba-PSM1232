...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    """ YOUR CODE HERE """
    max_inliers = -1
    for img1 in scene_graph:
        neighbors = scene_graph[img1]

        for img2 in neighbors:
            matches = load_matches(image_id1=img1, image_id2=img2)
...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    """ YOUR CODE HERE """
    # 使用recoverPose从本质矩阵中恢复旋转矩阵和平移向量
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    # 将3D点投影回2D图像平面
    projected_pic, _ = cv2.projectPoints(points3d, rvec, tvec, intrinsics, distCoeffs=None)
    
    # 计算投影点与观测点之间的欧氏距离
    residuals = np.linalg.norm(points2d - projected_pic.squeeze(), axis=1)
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    max_valid_idx = len(all_points3d) - 1  
    valid_mask = point3d_idxs <= max_valid_idx  # 筛选有效索引的掩码
    # 过滤2D和3D索引
    point3d_idxs = point3d_idxs[valid_mask]
    point2d_idxs = point2d_idxs[valid_mask]
    num_pts = point2d_idxs.shape[0]  # 更新点数
    ...
    for _ in range(num_ransac_iterations):
        ...
        rotation_mtx, tvec = np.eye(3), np.zeros(3, dtype=float)  
       ...
       # 调用solvePnP解位姿
        ...
        if success:
            ...
            rotation_mtx, _ = cv2.Rodrigues(rvec)            # 将旋转向量转回旋转矩阵
            # 计算重投影残差
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    # 获取外参
    ext1 = all_extrinsic[image_id1]
    ext2 = all_extrinsic[image_id2]
    
    new_points3d = triangulate(image_id1, image_id2, matches[:, 0], matches[:, 1], ext1, ext2, intrinsics)    # 对未注册的匹配点进行三角化
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    for reg_id in registered_ids:
        for neighbor_id in scene_graph[reg_id]:
